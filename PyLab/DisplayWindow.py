...
class CameraWidget(QWidget):
    ...
    def update_image_info(self,data):
        ...
        if 'fit1D_x' in data.__dict__:
            ...
            if Nxpx != Nxpx:
                Nxpx = 0
            X0 = int(data.fit1D_x[1]+0.5)+int(self.roi.pos()[0])
            W0 = data.fit1D_x[2]
            if W0 != W0:
                W0 = 0
            Nx = self.nAtoms(Nxpx)/1e6
            Wx = self.realSize(W0)
        if 'fit1D_y' in data.__dict__:
            Nypx = data.fit1D_y[0]
            if Nypx != Nypx:
                Nypx = 0
            Y0 = int(data.fit1D_y[1]+0.5)+int(self.roi.pos()[1])
            W0 = data.fit1D_y[2]
            if W0 != W0:
                W0 = 0
            Ny = self.nAtoms(Nypx)/1e6
            Wy = self.realSize(W0)
        n = 0
        T = 0
        if Wx and Wy:
            n = (Nx*Ny)**0.5/(4*np.pi/3*Wx*Wx*Wy)*1e24
            T = 3.3125*(1.055e-34)**2*n**(2/3)/1.38e-23/169/1.67e-27*1e9
            n = math.log10(n/1e6)
        self.image_data_to_display['x'] = np.array([int(Nxpx+0.5),X0,'%.2f'%Nx,int(Wx+0.5),int(n)],dtype=object)
        self.image_data_to_display['y'] = np.array([int(Nypx+0.5),Y0,'%.2f'%Ny,int(Wy+0.5),int(T)],dtype=object)
        ...
        # else:
        #     self.image_data_to_display['2D'] = np.array(['-'] * 6, dtype=object)
    ...
    def updateImage(self):
        ...
        self.original_data = np.array(self.parent.globals[self.image_name][self.image_bounds[0][0]:self.image_bounds[0][1],
                                                                           self.image_bounds[1][0]:self.image_bounds[1][1]])
        # better to save original
        self.new_data = impr.Image_Basics(self.original_data, subs_bgnd=self.subs_bgnd)
...
class DisplayWidget(da.DockArea):
    ...
    def __init__(self, parent=None, globals=None, signals=None, **kwargs):
        ...
        self.parent = parent
        self.config_file = os.path.join('display_widget_config.json')
    ...
    def routine(self):
        ...
        if len(self.globals['image_stack']): # if scan is on images have to be saved
            ...
            z = (4095*self.cam1.original_data).astype(np.uint16)
            # with open(im_name, 'wb') as f:
            #     writer = png.Writer(width=z.shape[1], height=z.shape[0],
            #                         bitdepth=12, greyscale=True)
            #     writer.write(f, z.tolist())
...
class MeasurementClass(QWidget):
    ...
    def saveFigandData(self):
        ...
        navrD = impr.mod_avrData({key:{0:self.dataD[key]['avr']} for key in self.dataD},
                                 self.xaxis_calib,
                                 impr.N_atoms(gain=324, exposure=909, power=2.55, width=2.43, delta=7e6),
                                 impr.real_size)
