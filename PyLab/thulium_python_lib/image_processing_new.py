...
Last modified 29.07.2016
    two gaussian fit and plot_image_1D_projection function added
...
def two_gaussian_fixed_N_cool(x, N2, x0, sigma1, sigma2, background):
    return two_gaussian(x, two_gaussian_fixed_N_cool.N1, N2, x0, sigma1, sigma2, background)
...
def two_gaussian2_fixed_N_cool(x, N, x0, sigma1, sigma2, background):
    return two_gaussian2(x, N, two_gaussian2_fixed_N_cool.alpha_cool, x0, sigma1, sigma2, background)
...
class Image_Basics():
    ...
    def __init__(self,image, image_url='not_defined',subs_bgnd=True):
...
#         (data.folderN, data.shotN, data.shot_typeN) = map(float, findall(r"[-+]?\d*\.\d+|\d+", image_url)[-3:])
        ...
        try: 
            ...
            if self.do_fit1D_x and self.do_fit1D_y:
                data.total_small = sum(data.image[int(data.fit1D_y[1]-self.n_sigmas*data.fit1D_y[2]):int(data.fit1D_y[1]+self.n_sigmas*data.fit1D_y[2]),
                                                 int(data.fit1D_x[1]-self.n_sigmas*data.fit1D_x[2]):int(data.fit1D_x[1]+self.n_sigmas*data.fit1D_x[2])])
        except RuntimeError:
...
def mod_avrData(avr_dataD, folderN_calib,n_atom_calib, lin_dim_calib):
    """ modifies avr_dataD dictionary with
        1 Calibrating x-value(foldeN) in respect with measured (i.e. changes it to Gs or MHz value)
        2 Drops from data image attribute (image itself is not needed for futher analysis)
        4 Changes numbers of atoms in fits and totals and linear dimentions (line x0 and width) from pixels to real
            size in μm
    """
    def mod_val(key, value):
        ...
        elif key.startswith("fit_two_gaussian1D"):
            val[0] = n_atom_calib(val[0])
#             val[1] = n_atom_calib(val[1])
            val[-1] = n_atom_calib(val[-1])
            val[2:-1] = lin_dim_calib(val[2:-1])
            return val
...
class N_atoms:
  ...
  [angle]=1
  """
  def __init__(self, gain=324, exposure=909, power=2.42, width=2.43, delta = 7e6, gamma = 1e7, angle = 0.01962/4/np.pi, Isat = 0.189, hw = 1.055*10**(-34)*2*np.pi*3*10**8/(410*10**(-9))):
    self.s = 2*2*power*(2.3/2.55)**0.5/np.pi/(width/2)**2/Isat
    self.rho = self.s/2/(1+self.s+(2*delta/gamma)**2)
    self.p = (2.55/2.30)**(0.5)*1.21197382e-7/exposure/np.e**(0.0051*gain)/gamma/hw/angle/self.rho
...
def real_size(x, binning=2, pixel_size = 6.459):
...
def clock(conf_params):
    x_lbl = 'Laser detuning, kHz'
    return x_lbl, y_lbl_default, lambda y: 2*y
...
def get_pandas_table(navrD):
    ...
    def gen_indexs(keys):
        ...
        tr_table = {'fit1D':['N','x0','sigma','bgnd'], 
                    ...
                    'fit_two_gaussian1D':['N1','N2','x0','sigma1','sigma2','bgnd'],
                    'center_pos':['x','y']}
        for key in keys:
...
def two_gaussian_fit(avr_dataD,fix_N_cool=True,p00=[100,50, 100,10,50,0]):
    """First version of two gaussian fit. Can only fix cold atoms, tha nis not cood due to decreace in atomes number 
    with time. It's better to use two_gaussian_fit2"""
    from scipy.optimize import curve_fit
    from numpy import inf
    shot_typeN = 1
    first_image = True
    p0s = [p00,p00]
    for key, item in avr_dataD.items():
        im = item[shot_typeN].image
        ...
        for i in (0,1):
            ...
            if first_image:
                ...
                if fix_N_cool:
                    two_gaussian_fixed_N_cool.N1 = p0s[i][0]
                    fit_func = two_gaussian_fixed_N_cool
                    p0 = copy(p0s[i][1:])
                else :
                    fit_func = two_gaussian
            popt, pcov = curve_fit(fit_func, xx, prog, p0=p0,bounds=(0,inf))
            setattr(item[shot_typeN],'fit_two_gaussian1D_' + ('x' if not i else 'y'), 
                    append([p0s[i][0]],popt) if (fix_N_cool and not first_image) else popt)
            if first_image:
                p0s[i]=popt
        first_image = False
    return avr_dataD
...
def two_gaussian_fit2(avr_dataD, fix_N_cool=True, p00=[20,0.8, 80,4,50,0]):
    """Second version of two gaussian fitting of cloud. Gets avr_dataD dictionary, modifies it, fix_N_cool is flag if to 
    fix fraction [0,1] of cold atom (recommended to True) and p00 is first guess in fittind 
    [N_atoms, cold_atoms_fraction, cloud_center, cold_fraction_width, hot_fraction_width, background]
    !! For now works only for short_typeN=, if neaded can be extended"""
    from scipy.optimize import curve_fit
    from numpy import inf
    shot_typeN = 1
    # if first folder than fits cloud shape with all free parameters, 
    # else fixed alpha_cool to the value obtained for first folder
    first_image = True
    # for x and y progections
    p0s = [p00,p00]
    alpha0=[0,0]
    alpha0err = [0,0]
    for key, item in avr_dataD.items():
        ...
        for i in (0,1):
            ...
            prog = sum(im,i)
            xx = arange(len(prog))
#             fit_func = two_gaussian2
            # now choose which function to use
            if first_image:
                fit_func = two_gaussian2
            else :
                if fix_N_cool:
                    # save alpha_cool as function attribute to generate function. It can be done better
                    two_gaussian2_fixed_N_cool.alpha_cool = alpha0[i]
                    fit_func = two_gaussian2_fixed_N_cool
                else :
                    fit_func = two_gaussian2
            popt, pcov = curve_fit(fit_func, xx, prog, p0=p0,bounds=(0,inf))
            perr = np.sqrt(np.diag(pcov))
            setattr(item[shot_typeN],'fit_two_gaussian1D_' + ('x' if not i else 'y'), 
                    insert(popt,1,alpha0[i]) if (fix_N_cool and not first_image) else popt)
            setattr(item[shot_typeN],'fit_two_gaussian1D_' + ('x' if not i else 'y')+'_std', 
                    insert(perr,1,alpha0err[i]) if (fix_N_cool and not first_image) else perr)
            if first_image:
                # save alpha_cool coeffitient
                alpha0[i]=popt[1]
                alpha0err[i] = perr[1]
                # construct new p0s with 1 droped parameter
                p0s[i]=copy(append(popt[0],popt[2:]))
            else:
                p0s[i]=popt
        first_image = False
    return avr_dataD
...
def plot_image_1D_projection(data, image_axis=0):
    """Function to display 1D plot and fits. data - avr_dataD[folderN][shot_typeN], image_axis 0 for 'x' and 1 for 'y'"""
    from matplotlib.pyplot import plot
    im = sum(data.image,image_axis)
    xx = arange(len(im))
    plot(xx,im)
    try:
        popt = data.fit_two_gaussian1D_x if not image_axis else data.fit_two_gaussian1D_y
        plot(xx,two_gaussian2(xx,*popt),'k')
        plot(xx, gaussian(xx,popt[0]*popt[1],popt[2],popt[3],popt[-1]),'r')
        plot(xx, gaussian(xx,popt[0]*(1-popt[1]),popt[2],popt[4],popt[-1]),'b')
        print('two_gaussian popt',popt)
    except AttributeError:
        print("Image hasn't two gaussian fit")
    fit_func = gaussian
    popt = data.fit1D_x if not image_axis else data.fit1D_y
    plot(xx,fit_func(xx,*popt),'y')
    print('single_gaussian popt',popt)
