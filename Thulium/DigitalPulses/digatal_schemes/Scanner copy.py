...
class Scanner(QWidget):
    ...
    class SingleScanParameter():
        ...
        short_name = ''
        nesting = None  # indicate at which layer of nesting it is
        param_list = []
        current_index = 0
    ...
    class AllScanParameters():
        ...
        def addParameter(self, parameter):
            ...
            self.params_list[parameter.nesting].append(parameter)

        @property
        def full_nesting(self):
            return len(self.params_list) - 1
    ...
    def initUI(self):
        ...
        print('Done')
    scan_grid_elems={'menu':0,'short':1,'index':2,'line':3,'current':4,'del':5,'add':6}
    def scanDraw(self):
        print('scanDraw')
        grid =  QGridLayout()
        row = 0
        for i,param_level in enumerate(self.scan_params):
            print(param_level)
            for j,param in enumerate(param_level):
                ...
                self.generateParamMenu(param_menu, self.all_scan_params)
                menu_btn.setMenu(param_menu)
                grid.addWidget(menu_btn,row,self.scan_grid_elems['menu'])

                param_shortname = QLineEdit('')
                param_shortname.setMaximumWidth(20)
    ...
    def updateSingleMeasFolder(self):
        ...
        vars = []
        print(self.scan_params)
        for item in self.scan_params[0]:
            # print(item)
            vars.append(self.current_params[tuple(item[0])])
        self.single_meas_folder = ' '.join([str(elem) for elem in vars])+single_folder_suffix
        self.updateGlobals()
    ...
    def updateParamAndSend(self,changed_index):
        ...
        for i in range(changed_index + 1): # to handle changed index as well
            for param in self.scan_params[i]:
                ...
                lbl = grid.itemAt(iterator * len(self.scan_grid_elems) + self.scan_grid_elems['current']).widget()
                print(lbl)
                lbl.setText(str(param[1][self.current_param_indexs[i]]))
                lbl.repaint()
                iterator +=1
