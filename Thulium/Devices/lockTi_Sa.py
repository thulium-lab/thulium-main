...
class LockTi_Sa(QWidget):
    config_file = 'lockTi_Sa.json'
    channel = 'Ti:Sa'
    frequency = 357850570.0 # MHz
    threshold = 2 # MHz
    gain = 1.0  # mV/MHz
    ...
    def __init__(self):
        ...
        self.setWindowTitle('Ti:Sa Laser Lock')
    ...
    def initUI(self):
        ...
        self.K_p_line.setDecimals(2)
        self.K_p_line.setRange(-99999, 99999)
        self.K_p_line.setSingleStep(1)
        ...
        self.K_i_line.setDecimals(2)
        self.K_i_line.setRange(-99999, 99999)
        self.K_i_line.setSingleStep(1)
        ...
        self.K_d_line.setDecimals(2)
        self.K_d_line.setRange(-99999, 99999)
        self.K_d_line.setSingleStep(1)
    ...
    def routine(self):
        ...
        #error = 3.0 - self.output_voltage
        #print('error',error)
        self.error_log.append(error)
        self.output_plots.error_curve.setData(self.error_log)
        if abs(error) > self.threshold:
            self.P = self.gain*error*1e-3
            self.I += self.gain*error*1e-3
            self.D = self.gain * (error - self.error2)*1e-3
            self.error2 = error

            #print ("P = ", self.P, "I = ", self.I, "D = ", self.D)
            self.output_voltage = self.initial_voltage + self.K_p * self.P + self.K_i * self.I + self.K_d * self.D #self.gain*error*1e-3
