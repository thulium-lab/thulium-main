...
class WMChannel:
    ...
    class WMChannelGUI(QWidget):
        def __init__(self, parent=None, data=None):
            ...
            self.active_check_box = QCheckBox(text='Active')
            self.active_check_box.setChecked(self.data.is_active)
            self.active_check_box.toggled.connect(self.setActive)
            layout1.addWidget(self.active_check_box)
        ...
        def setSingle(self,b):
            print('Single changed')
            self.data.single = b

            # p = self.palette()
            # p.setColor(self.backgroundRole(), Qt.red if b else Qt.blue)
            # self.setPalette(p)
        ...
        def setActive(self,b):
            self.data.is_active = b
            self.parent.drawChannels()
            # self.parent.updateSingleChannel(self.data.name, self.data.single)
...
class WMMain():
    ...
    def __init__(self, arduino=None):
        ...
        self.load()
        self.arduino = arduinoShutters.Arduino(port=self.config.get('port','COM12'))
    ...
    def save(self):
        ...
        data_to_save['mode'] = self.mode
        data_to_save['single_index'] = self.single_index
    ...
    def load(self,file_name='WM_config.json'):
        ...
        if 'single_index' in data:
            self.single_index = data['single_index']
        if 'mode' in data:
            self.mode = data['mode']
    ...
    class WMWidget(QWidget):
        ...
        def __init__(self, parent=None, data=None, signals=None):
            ...
            self.calibrateBox.setChecked(False)
        ...
        def routine(self):
            ...
            if self.data.mode == 'all':
                if self.data.current_index not in self.data.active_channels_indexes:
                    self.data.current_index=self.data.active_channels_indexes[0]
        ...
        def drawSpecta(self):
            ...
            # print("Index",self.data.single_index, self.data.mode)
            if self.data.mode == 'single':
                active_channels = [self.data.channels[self.data.single_index]]
            else:
                active_channels = [channel for channel in self.data.channels if (channel.is_active  and channel.show_spectrum)]
            # active_channels = [channel for channel in self.data.channels if
            #                    (channel.is_active and channel.show_spectrum)]
        ...
        def drawChannels(self):
            ...
            for i, channel in enumerate(self.data.channels): #enumerate(active_channels):
                # print(i % 2, i // 2)
